...
ord_prob <- function(cats, seed, ...) {
    vars  <- data.frame(...)
    set.seed(seed)
    coefs <- matrix(runif((cats - 1) * ncol(vars), -1, 1), ncol = cats - 1)
...
D <- DAG.empty() +
    node("L", t = 1, distr = "rcat.b1",
         probs = ord_prob(3, 3234, 1)) +
    node("A", t = 1, distr = "rcat.b0",
         probs = ord_prob(2, 982365, L[t])) +
    node("Z", t = 1, distr = "rcat.b1",
         probs = ord_prob(3, 9865, L[t], A[t])) +
    node("M", t = 1, distr = "rcat.b1",
         probs = ord_prob(4, 5109865, Z[t], L[t], A[t])) +
    node("L", t = 2, distr = "rcat.b1",
         probs = ord_prob(3, 323714, M[t-1], Z[t-1], L[t-1], A[t-1])) +
    node("A", t = 2, distr = "rcat.b0",
         probs = ord_prob(2, 825, L[t], M[t-1], Z[t-1], L[t-1], A[t-1])) +
    node("Z", t = 2, distr = "rcat.b1",
         probs = ord_prob(3, 65, L[t], A[t], M[t-1], Z[t-1], L[t-1], A[t-1])) +
    node("M", t = 2, distr = "rcat.b1",
         probs = ord_prob(4, 10965, Z[t], L[t], A[t], M[t-1], Z[t-1], L[t-1], A[t-1])) +
    node("Y", t = 3, distr = "rcat.b0",
         probs = ord_prob(2, 51865, M[t-1], Z[t-1], L[t-1], A[t-1], M[t-2], Z[t-2], L[t-2], A[t-2]))
...
true <- function() {
    ...
    datx <- mutate(data_points,
                   QL2 = ord_prob(2, 51865, M_2, Z_2, L_2, A_2, M_1, Z_1, L_1, A_1)[, 2])

    pZ2 <- with(datx, ord_prob(3, 65, L_2, A_2, M_1, Z_1, L_1, A_1))
    ...
    pL2 <- with(datx, ord_prob(3, 323714, M_1, Z_1, L_1, A_1))
    ...
    pZ1 <- with(datx, ord_prob(3, 9865, L_1, A_1))
    ...
    pM2 <- with(data_points, ord_prob(4, 10965, Z_2, L_2, A_2, M_1, Z_1, L_1, A_1))
    ...
    pM1 <- with(datx, ord_prob(4, 5109865, Z_1, L_1, A_1))
    ...
    return(c(theta11, theta10, theta01, theta00))
